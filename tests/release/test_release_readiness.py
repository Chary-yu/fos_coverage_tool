...
class ReleaseReadinessTest(unittest.TestCase):
    ...
    def _inputs(self):
        ...
        return matrix, tasks, risk

    def test_missing_gate_or_risk_provenance_is_not_ready(self):
        matrix, tasks, _ = self._inputs()
        result = build(self.repo_root, matrix=matrix, task_status=tasks, risk_register=None)
    ...
    def test_all_gates_and_empty_risk_register_can_be_ready(self):
        matrix, tasks, risk = self._inputs()
        result = build(self.repo_root, matrix=matrix, task_status=tasks, risk_register=risk)
