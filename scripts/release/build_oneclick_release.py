...
def _script(metadata, final_sha, final_tree, baseline_sha, bundle_sha,
    ...
    lines = [
        ...
        "  fi",
        "  if [ -f \"$STATE_ROOT/upgrade-state.json\" ]; then",
        "    echo; cat \"$STATE_ROOT/upgrade-state.json\"",
        "  fi",
        ...
        "  VERIFY_REPO=\"$VERIFY_ROOT/verify-repo\"",
        "  mkdir -p \"$VERIFY_REPO\"",
        "  BUNDLE_ABS_PATH=\"$(cd \"$(dirname \"$BUNDLE_PATH\")\" && pwd -P)/$(basename \"$BUNDLE_PATH\")\"",
        "  (cd \"$VERIFY_REPO\" && git init -q && git bundle verify \"$BUNDLE_ABS_PATH\" >/dev/null)",
        "  git clone \"$BUNDLE_ABS_PATH\" \"$VERIFY_ROOT/source\" >/dev/null",
        ...
        "# Production defaults to /usr/bin/python3; it must report Python 3.6 (python3.6-compatible).",
        "PYTHON_BIN=\"${FOS_R8_PYTHON:-/usr/bin/python3}\"",
        "if ! command -v \"$PYTHON_BIN\" >/dev/null 2>&1 && [ ! -x \"$PYTHON_BIN\" ]; then",
        "  echo \"Python 3.6 executable is required (default: /usr/bin/python3): $PYTHON_BIN\" >&2; exit 1",
        "fi",
