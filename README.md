...
当前文件填写多个控件后，可使用右下角“定位首个待填写”跳转到第一个未确认或暂存控件；随后可在面板中使用“上一个/下一个”继续跳转。填写完成后，可使用“暂存草稿”一次批量写入数据库；草稿允许保留“未确认”及未完成字段，计入填写进度但不计入已确认结论。“确认提交”会对待暂存控件执行状态、确认人和覆盖说明校验后再批量提交；离开含未暂存内容的页面会收到提示。
