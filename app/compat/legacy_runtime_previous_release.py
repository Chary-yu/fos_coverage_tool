...
from app.incremental import legacy as coverage_check
from app.code_detail.code_region import CodeRegion, FunctionRange, build_code_regions
...
from app.code_detail.service import CodeDetailService, is_safe_relative_path
...
# Compatibility names above are intentionally re-bound to the one canonical
# registry implementation before any legacy command can call them.  Keeping
# these names preserves old imports without retaining a second state machine.
from app.reports.registry import ReportRegistry as _CanonicalReportRegistry


def _canonical_report_registry(registry_dir=None):
    return _CanonicalReportRegistry(
        registry_dir or get_configured_registry_dir(),
        legacy_path=REPORT_REGISTRY_LEGACY_PATH,
    )


def prune_stale_report_registry(registry_dir: Optional[str] = None):
    return _canonical_report_registry(registry_dir).prune()


def register_report_directory(report_id: str, *dirs: str, **metadata):
    return _canonical_report_registry().register(report_id, dirs, **metadata)


def load_report_registry(report_id: Optional[str] = None) -> Dict[str, List[str]]:
    registry = _canonical_report_registry()
    if report_id:
        item = registry.load_exact(report_id)
        return {report_id: list(item.get("directories") or [])} if item else {}
    return {
        key: list(item.get("directories") or [])
...
def incremental_file_key(repository_name, file_path):
    """Keep identical paths in independent repositories distinct in the UI."""
    repository_name = str(repository_name or "").strip()
    normalized = _normalize_ownership_path(file_path)
    return "{}::{}".format(repository_name, normalized) if repository_name else normalized
        ...
        for row in rows:
            ...
            if isinstance(row, dict) or hasattr(row, "keys"):
                try:
                    ...
                    repository_name = row.get("repository_name", "") if isinstance(row, dict) else (
                        row["repository_name"] if "repository_name" in row.keys() else ""
                    )
                ...
                continue
            if fpath:
                file_key = incremental_file_key(repository_name, fpath)
...
def sync_incremental_unanalyzed_counts(project_name, result, config=None):
    ...
    for repository_name, review_file_path in sorted(details_by_file):
        ...
        file_key = incremental_file_key(repository_name, review_file_path)
...
def write_incremental_summary_page(output_html_dir, project_name, result, config=None, unanalyzed_by_file=None):
    ...
    if repositories:
        git_range_text = "多仓库联合审查 ({})".format(len(repositories))
...
def write_incremental_developer_tasks_page(output_html_dir, project_name, result):
    ...
    for developer in developers:
        ...
        for file_task in developer.get("files") or []:
            ...
            file_key = incremental_file_key(file_task.get("repository"), source_path)
