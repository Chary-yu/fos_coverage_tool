...
`app/release_identity.py` is the canonical release identity owner. A release identity contains `version`, exact `commit_sha`, `build_id`, static `asset_hash`, `schema_version` and build provenance time. The build step writes `release_manifest.json`; runtime verification never rewrites a missing or drifting manifest.
